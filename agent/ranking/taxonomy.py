...
TECH_TAXONOMY: Dict[str, Tuple[FrozenSet[str], str]] = {
    ...
    "langchain":     (frozenset({"langchain-core", "langchain community", "lcel", "langchain python"}), "ai"),
    "llm":           (frozenset({"large language model", "gpt", "claude", "gemini", "llama", "mistral", "openai api", "anthropic api", "llm api"}), "ai"),
    "rag":           (frozenset({"retrieval augmented generation", "retrieval-augmented", "vector search", "rag pipeline"}), "ai"),
    "vector_db":     (frozenset({"lancedb", "pinecone", "weaviate", "chroma", "qdrant", "pgvector", "milvus", "vector database"}), "ai"),
    "mlops":         (frozenset({"llmops", "ml ops", "model serving", "mlflow", "wandb", "weights & biases", "model registry"}), "ai"),
    "computer_vision": (frozenset({"cv", "opencv", "object detection", "yolo", "image classification", "segmentation"}), "ai"),
    "nlp":           (frozenset({"natural language processing", "text classification", "named entity", "ner", "sentiment analysis"}), "ai"),
